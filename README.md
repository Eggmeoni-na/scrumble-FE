...
- **아이콘용으로 사용할 svg 파일을 정적 자산이 아닌 컴포넌트 관리했습니다.**
    - 경로를 전달하는 것보다 컴포넌트를 이용하여 선언적으로 사용하는 것이 더 낫다고 판단했습니다.
    - 정적 파일보다 무거워질 수 있지만, 서비스 내에서 필요한 아이콘의 갯수가 많지 않았습니다.
    - Vite의 **svgr 플러그인**을 이용해 컴포넌트화하여 사용성을 높였습니다.
<table>
  <tr>
    <th colspan="2">투두 생성/수정/삭제의 불필요한 API 호출</th>
  </tr>
  <tr>
    <th width="120px">항목</th>
    <th width="1080px">내용</th>
  ...
  <tr>
    ...
    <td>
      <ol>
        <li><strong>사용성 개선</strong>: 투두 생성, 수정, 삭제 시 리페치 없이 변경된 데이터를 즉시 반영하여 사용자 경험 향상</li>
        <li><strong>API 호출 최소화</strong>: 날짜별 데이터를 캐싱하여 동일 날짜에 대한 API 재호출 방지</li>
        <li>유틸 함수 분리로 <strong>가독성과 유지보수성</strong> 향상</li>
      </ol>
    </td>
  </tr>
</table>
...
- **스쿼드 초대 알림 구현 필요성**
    ...
    - **실시간 확인**: 온라인 상태에서 발생한 알림을 즉시 확인 가능
    - **오프라인 상태 보장**: 오프라인 중 수신된 알림을 온라인 상태에서 확인 가능
- **기술 선택 및 검토**
    1. **폴링(Polling)**
        - **장점**: 구현이 간단하며 HTTP 기반으로 어디서나 사용 가능
        - **단점**: 주기적으로 서버에 요청을 보내 헤더 중복 및 불필요한 트래픽이 발생하여 서버 부하 증가
    2. **롱폴링(Long Polling)**
        - **장점**: 폴링 단점을 보완하여 서버에서 응답 시까지 연결을 유지해 불필요한 요청 감소
        - **단점**: 서버에서 응답을 보낸 후 연결이 종료되어 새로운 데이터가 필요하면 재연결 필요
    3. **웹소켓(WebSocket)**
        - **장점**: 전이중 통신으로 실시간 양방향 데이터 전송 가능, 헤더 중복 문제 없음
        - **단점**: 지속적인 연결 유지로 서버 리소스 부담, 단순 단방향 통신에 비해 복잡성 증가
    4. **FCM(Firebase Cloud Messaging)**
        - **장점**: 모바일 기기 대상 푸시 알림에 적합, 다양한 플랫폼 지원
        - **단점**: 웹 서비스에서의 접속 기기별 알림 동기화 문제
